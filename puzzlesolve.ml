...
module MakePuzzleSolver
  ...
  struct
      ...
      let solve : unit -> move list * state list =
         ...
         let rec add_neighbors (nghbr: (state * move) list)
                               (pndng : Pending.collection)
                               (mvlst : move list)
                               (vstd : Visited.t) : Pending.collection =
            match nghbr with
            | (s, m) :: tl -> if Visited.mem s vstd 
                              then add_neighbors tl pndng mvlst vstd
                              else add_neighbors tl (Pending.add (s, mvlst @ [m]) pndng) mvlst vstd
            | [] -> pndng in

         let rec solver (pndng : Pending.collection) 
                        (vstd : Visited.t) : move list * state list =
            if Pending.is_empty pndng then raise CantReachGoal
            else
               let (stat, mvlst), remaining = Pending.take pndng in
                  if Puzzle.is_goal stat then (mvlst, Visited.elements (Visited.add stat vstd))
                  else if Visited.mem stat vstd then
                     solver remaining vstd
                  else 
                     let nghbr = Puzzle.neighbors stat in
                     solver (add_neighbors nghbr pndng mvlst vstd) (Visited.add stat vstd) in
            fun () -> solver pending visited
         
         let draw (stlst : state list) (mvlst : move list) : unit =
            Puzzle.draw stlst mvlst

         let print_state (st : state) : unit =
            Puzzle.print_state st
